...
//
const menuPortfolio = (title: string) => {
  activeMenuPortfolio.value = title;
  router.push({ path: '/portfolio', query: { cat: encodeURI(title) } });
};
...
watchEffect(() => {
  if (!route.path.includes('portfolio')) {
    ...
    activeMenuPortfolio.value = decodeURI((route.query.cat as string) || '');
  }
});
</script>
